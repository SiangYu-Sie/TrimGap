...
namespace CHRocodileLib
{
    ...
    public class Connection : IDisposable
    {
        ...
        public void SetOutputDataFormatMode(Int32 mode)
        {
            ...
            Int64 endPos = 0, bool activateAutoBufferMode = true)
        {
            if (_curDataInfo == null)
                _curDataInfo = new DataInfo(Handle);
            if (_curDataInfo.SizePerSample == 0)
                    throw new Error("InitializeNewBuffer error!");
            ...
            // copy existing data (from GetNextSamples() call) into buffer:
    ...
    public class SynchronousConnection : SynchronousCommandExecution
    {
        ...
        /// <param name="deviceType"></param>
        /// <param name="devBufSize"></param>
        public SynchronousConnection(string connectionInfo, DeviceType deviceType, Int64 devBufSize = 0)
            : base(connectionInfo, deviceType, devBufSize)
        {
            Open();
        ...
        public override Response Exec(Cmd_h cmdHandle)
        {
            lock (_recordingLock)
            {
                ...
                return new Response(rsp, Handle);
            }
        }
    ...
    public partial class AsynchronousConnection : AsynchronousCommandExecution
    {
        ...
        private Lib.ResponseAndUpdateCallback _libSpecCmdCb;    // special user callback
        ...
        {
            if (DataCb == null || sizePerSample == 0)
                return;
            ...
            lock (this) // TODO: other lock object
            {
                try
                {
                    ...
                    if (data.SynchronizeEndPos(0))
                        DataCb(status, data);
                    if (DataCb == null) // data flow can be reset inside DataCb!
                        return;
                    mustStartNewBuffer |= (status != AsyncDataStatus.TimeOut)
                        && (status != AsyncDataStatus.Success)
                        && (status != AsyncDataStatus.BufferLimitHit);
        ...
        private void CmdGenCbFct(TRspCallbackInfo _sInfo, Rsp_h _hRsp)
        {
            lock (this)
            {
                try
                {
                     ...
                     */
                    _genCmdCb?.Invoke(new Response(_hRsp, _sInfo.Source, false));
                }
                catch (Exception ex)
                {
                    LastException = ex;
        ...
        /// <summary>
        /// Used internally. Wrapper around user-defined callback functions
        /// that translates the DLL "user pointer" back to the delegate which in turn
        /// gets called from inside a try / catch block
        /// </summary>
        /// <param name="_sInfo">The info struct</param>
        /// <param name="_hRsp">The response handle used to extract the actual response</param>
        private void CmdSpecialCbFct(TRspCallbackInfo _sInfo, Rsp_h _hRsp)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ...
                    ResponseAndUpdateCallback cb = handle.Target as ResponseAndUpdateCallback;
                    var rsp = new Response(_hRsp, _sInfo.Source, false);
                    cb.Invoke(rsp);
                }
                catch (Exception ex)
                {
        ...
        /// It is also used as a backend for other variants such as "Exec(CmdID.ScanRate, ...)"
        /// </summary>
        /// Examples:
        /// <code>
        ///     Lib.NewCommandFromString("SHZ 4000", out var handle);
        ...
        ///     Lib.NewCommand("SHZ", false, out var handle);
        ///     Lib.AddCommandFloatArg(handle, 4000.0);
        ///     var ticket = Exec(handle, rsp => Console.WriteLine($"Response: {rsp}"));
        /// </code>
        /// <param name="cmdHandle">Command handle as returned by Lib.NewCommand functions</param>
        /// <param name="userCb">User callback function to be called once the response is ready</param>
        /// <returns>ticket to later identify the response</returns>
        public override int Exec(Cmd_h cmdHandle, ResponseAndUpdateCallback userCb)
        {
        ...
        public class Plugin : AsynchronousCommandExecution
        {
                ...
                lock (this)
                {
                    try
                    {
                        if (_sInfo.User != IntPtr.Zero)
                        {
                             ...
                             * Since this is typically executing in a separate thread, throwing is not straight-forward.
                             */
                            var func = Marshal.GetDelegateForFunctionPointer<ResponseAndUpdateCallback>(_sInfo.User);
                            var rsp = new Response(_hRsp, _sInfo.Source, false);
                            func.Invoke(rsp);
                        }
                    }
            ...
            public override int Exec(Cmd_h cmdHandle, ResponseAndUpdateCallback userCb)
            {
                var puser = userCb != null ? Marshal.GetFunctionPointerForDelegate(userCb) : IntPtr.Zero;
                _libSpecCmdCb = CmdSpecialCbFct; // keep reference to the delegate to prevent gabrage collection
                Lib.Check(Lib.ExecCommandAsync(Handle, cmdHandle, puser, _libSpecCmdCb, out var ticket));
                return ticket;
            }
        }
