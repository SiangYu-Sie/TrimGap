        ...
        static void Main(string[] args)
        {
            ...
            using (var axis = new Axis() { Con = con })
            {
                try
                {
                    ...
                    con.DetachRenewDataBuffer(scanSampleCount); // remove old samples, begin new buffer for 1000 samples
                    state = State.Running;
                    axis.MoveToEnd();
                    ...
                    foreach (var s in outData.Samples())
                        d += s.Get(0);
                    Console.WriteLine($"Average distance = {d / outData.TotalNumSamples}");
                }
                catch (Exception e)
                {
