...
namespace TCHRLibBasicPullSample
{
    public partial class TPullSample : Form
    {
        ...
        int MeasuringMethod = CHRocodileLib.Lib.Confocal_Measurement;
        int[] SignalIDs;
        float ScanRate;
        ...
        private void SetUpMeasuringMethod()
        {
            try
            {
                int nMMD = CHRocodileLib.Lib.Confocal_Measurement;
                if (RBInterfero.Checked)
                    nMMD = CHRocodileLib.Lib.Interferometric_Measurement;
                var oRsp = Conn.Exec(CHRocodileLib.CmdID.MeasuringMethod, nMMD);
                MeasuringMethod = oRsp.GetParam<int>(0);
            }
            catch
            {
                Debug.Fail("Cannot set measuring method");
            }
            if (MeasuringMethod == CHRocodileLib.Lib.Confocal_Measurement)
        ...
        private void TTimerUpdate_Tick(object sender, EventArgs e)
        {
            ...
            if (TCData.SelectedTab == TPSpec)
            {
                try
                {
                    ...
                    if (RBCLS.Checked)
                        oRsp = Conn.Exec(CHRocodileLib.CmdID.DownloadSpectrum, SpecType, 0, 1);
                    else
        ...
        private void BtSend_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                switch (oRsp.Info.CmdID)
                {
                    ...
                    case (CHRocodileLib.CmdID.MeasuringMethod):
                        MeasuringMethod = oRsp.GetParam<int>(0);
                        if (MeasuringMethod == CHRocodileLib.Lib.Confocal_Measurement)
                            RBConfocal.Checked = true;
                        else
                    ...
                    case (CHRocodileLib.CmdID.ScanRate):
                        ScanRate = oRsp.GetParam<float>(0);
                        TBSHZ.Text = ScanRate.ToString();
                        break;
                    //In case of output signal response
                    case (CHRocodileLib.CmdID.OutputSignals):
                        SignalIDs = oRsp.GetParam<int[]>(0);
                        TBSODX.Text = String.Join(",", SignalIDs.Select(p => p.ToString()).ToArray());
                        break;
                    default:
