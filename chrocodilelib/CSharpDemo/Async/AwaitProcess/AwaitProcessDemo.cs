...
namespace TCHRLibAwaitProcess
{
    public partial class AwaitProcessDemo : Form
    {
        ...
        private async void SetupDevice()
        {
            try
            {
                //use await to wait for the response to come back, before next operation
                //here all the settings are pipelined, the next setting is started only after the previous one is finished
                ...
                var oRsp = await Conn.ExecAsync(CmdID.MeasuringMethod, Lib.Confocal_Measurement);
                TBMMD.Text = oRsp.GetParam<int>(0).ToString();

                //set output signals
                int[] signals;
                //CLS device only output 16 bit integer data
                if (RBCLS.Checked)
