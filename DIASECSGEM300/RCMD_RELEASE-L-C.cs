...
{
    public class RCMD_RELEASE_L_C
    {
        public string LOADPORT_ID { get; set; }
        public string CARRIER_ID { get; set; }

        public RCMD_RELEASE_L_C()
        {
    ...
    {
        public bool RunDecode(RemoteControlEventRemoteCommandArgs e, out RCMD_RELEASE_L_C rcmdInfo)
        {
            bool bDataCheckOK = true;
            rcmdInfo = new RCMD_RELEASE_L_C();
                    ...
                    case "CARRIER-ID":
                        {
                            if (obj.Format == ItemFmt.A)
                            {
                                string CARRIER_ID = (string)obj.Value;
                                if (CARRIER_ID.Trim() != string.Empty)
                                {
                                    //OK, Keep Data..
                                    rcmdInfo.CARRIER_ID = (string)obj.Value;
                                }
                                else
                                {
                                    //NG ,Reply Info..
